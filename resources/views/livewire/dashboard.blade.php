<div class="p-6 max-w-7xl mx-auto">
    ...
    <div class="mb-6 bg-white rounded-lg border border-gray-200 p-4">
        <form wire:submit="createPrompt" class="flex items-end gap-3">
            <div class="flex-1">
                ...
                @error('newPromptName') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Type</label>
                <select wire:model="newPromptType" class="rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm">
            ...
            <button type="button" wire:click="$toggle('showCreateForm')" class="px-4 py-2 text-gray-500 text-sm hover:text-gray-700">
                Cancel
            </button>
        </form>
    </div>
    ...
    {{-- Recent Prompts --}}
    <section class="mb-8">
        <h2 class="text-lg font-semibold text-gray-800 mb-3">Recent Prompts</h2>
        @if($recentPrompts->isEmpty())
            <p class="text-gray-500 text-sm">No prompts yet. Create one to get started.</p>
        @else
            ...
            <a href="{{ route('workspace', $prompt) }}" wire:navigate
               class="block bg-white rounded-lg border border-gray-200 p-4 hover:border-indigo-300 hover:shadow-sm transition">
                <div class="flex items-center gap-2 mb-1">
                    <h3 class="font-medium text-gray-900 truncate">{{ $prompt->name }}</h3>
                    @if($prompt->isFragment())
                        <span class="text-xs px-1.5 py-0.5 bg-amber-100 text-amber-700 rounded">fragment</span>
                    @endif
                </div>
    ...
    <section class="mb-8">
        <h2 class="text-lg font-semibold text-gray-800 mb-3">Starred Results</h2>
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            @foreach($starredResults as $result)
            <div class="bg-white rounded-lg border border-gray-200 p-4">
                <div class="flex items-center justify-between mb-1">
                    <span class="text-sm font-medium text-gray-900">{{ $result->provider_name ?? 'Manual' }}</span>
                    @if($result->rating)
                        <span class="text-xs text-amber-600">{{ str_repeat('★', $result->rating) }}</span>
                    @endif
                </div>
                <a href="{{ route('workspace', $result->prompt) }}" wire:navigate class="text-xs text-indigo-600 hover:text-indigo-800">
                    {{ $result->prompt->name }} v{{ $result->promptVersion->version_number }}
                </a>
                <p class="text-sm text-gray-600 mt-2 line-clamp-3">{{ Str::limit($result->response_text, 150) }}</p>
            </div>
            @endforeach
        </div>
    </section>
